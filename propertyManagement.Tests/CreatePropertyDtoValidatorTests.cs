...
/// <summary>
/// Unit tests for the <see cref="CreatePropertyDtoValidator"/> class.
/// </summary>
[TestFixture]
public class CreatePropertyDtoValidatorTests
{
    ...
    public void SetUp()
    {
        ...
        _validator = new CreatePropertyDtoValidator(_mockUnitOfWork.Object);
    }

    /// <summary>
    /// Verifies that a valid property creation DTO passes all validation rules.
    /// </summary>
    [Test]
    ...
    public async Task Validate_CityIdZeroOrNegative_Fails()
    {
        ...
        Assert.That(result.IsValid, Is.False);
        var errors = result.Errors.Select(e => e.ErrorMessage).ToList();
        Assert.That(errors, Contains.Item("City ID must be greater than zero."));
    }
